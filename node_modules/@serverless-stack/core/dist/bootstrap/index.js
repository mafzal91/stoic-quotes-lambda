...
import { synth, deploy, isRetryableException, STACK_DEPLOY_STATUS } from "../index.js";
...
export const assets = {};
export async function bootstrap(config, cliInfo, options) {
    const { region } = config;
...
function isVersionUpToDate(bootstrapVersion) {
    return parseInt(bootstrapVersion || "0") === parseInt(LATEST_VERSION);
}
export async function init(region) {
...
async function deployStack(config, cliInfo, tags) {
    const { region } = config;
...
function printStackDeployError(deployRet) {
