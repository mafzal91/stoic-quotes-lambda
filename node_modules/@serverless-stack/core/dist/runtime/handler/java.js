...
import { buildAsync, buildAsyncAndThrow } from "./definition.js";
export const JavaHandler = opts => {
    ...
    const target = path.join(dir, path.basename(opts.handler).replace(/::/g, "-"));
    const cmd = {
        command: buildBinary,
        args: [
            buildTask,
    ...
    return {
        ...
        run: {
            ...
            args: [
                "-cp",
                [
                    path.join(Paths.OWN_PATH, "../src/", "runtime", "shells", "java-bootstrap", "release", "*"),
                    path.join(target, "libs", "*"),
                    path.join(target, outputDir, "lib", "*"),
...
function getGradleBinary(srcPath) {
...
function getGradleBuildTask(bundle) {
    return (bundle && bundle.buildTask) || "build";
}
function getGradleBuildOutputDir(bundle) {
