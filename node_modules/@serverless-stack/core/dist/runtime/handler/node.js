...
function getModuleManager(srcPath) {
    let foundPath;
    ...
    return {
        ...
        bundle: async () => {
            ...
            try {
                ...
                throw new Error("There was a problem transpiling the Lambda handler: " + e);
            }
            await fs.writeJSON(path.join(artifact, "package.json"), {
                type: bundle.format === "esm" ? "module" : "commonjs"
            });
...
async function installNodeModules(srcPath, targetPath, bundle) {
    ...
    if (existingPath) {
        await fs.copy(path.join(existingPath, "node_modules"), path.join(targetPath, "node_modules"));
        return;
    }
    ...
    await fs.copy(path.join(lockFileDir, lockFile), path.join(targetPath, lockFile));
 ...
 */
async function extractDependencies(pkgPath, modules) {
    const dependencies = {};
...
async function runBeforeBundling(srcPath, buildPath, bundle) {
    // Build command
...
async function runBeforeInstall(srcPath, buildPath, bundle) {
    // Build command
...
async function runAfterBundling(srcPath, buildPath, bundle) {
    // Build command
